...
// Collect beneficiary counts (only non-empty).
// Default values sync dengan admin form (Beranda Transparansi).
...
for ( $i = 1; $i <= 5; $i++ ) {
        $count = (int) preg_replace( '/[^0-9]/', '', get_theme_mod( "alkautsar_beneficiary_{$i}_count", $beneficiary_defaults[ $i ]['count'] ) );
        $label = get_theme_mod( "alkautsar_beneficiary_{$i}_label", $beneficiary_defaults[ $i ]['label'] );
        if ( $count > 0 && $label ) {
                $beneficiaries[] = array( 'count' => $count, 'label' => $label );
                ...
                <?php if ( ! empty( $beneficiaries ) ) : ?>
                        <section style="margin-top:4rem;">
                                <div class="section-head section-head--center">
                                        ...
                                        <p class="section-desc"><?php esc_html_e( 'Ringkasan jumlah penerima manfaat per kategori.', 'alkautsar' ); ?></p>
                                </div>

                                <div class="beneficiary-stats">
                                        <?php foreach ( $beneficiaries as $b ) : ?>
                                                <div class="beneficiary-stat">
                                                        <span class="beneficiary-stat__number"><?php echo esc_html( $b['count'] ); ?></span>
                                                        <span class="beneficiary-stat__label"><?php echo esc_html( $b['label'] ); ?></span>
                                                </div>
                                        <?php endforeach; ?>
                                        <div class="beneficiary-stat beneficiary-stat--total">
                                                <span class="beneficiary-stat__number"><?php echo esc_html( $total_beneficiaries ); ?></span>
                                                <span class="beneficiary-stat__label"><?php esc_html_e( 'Total Penerima', 'alkautsar' ); ?></span>
                                        </div>
                                </div>
                        </section>
                <?php endif; ?>
