<?php
/**
 * Main index template — fallback for blog/news listing.
 *
 * @package AlKautsar
 */
	...
	<div class="container">
		<h1>
			...
			if ( is_home() && ! is_front_page() ) {
				...
				esc_html_e( 'Berita & Kabar', 'alkautsar' );
			}
			?>
		</h1>
		<?php
		if ( is_archive() && $desc = get_the_archive_description() ) {
			echo '<div class="breadcrumb">' . wp_kses_post( $desc ) . '</div>';
		}
		?>
	</div>
</header>
		...
		<?php if ( have_posts() ) : ?>
			...
			<div class="pagination">
				...
				echo paginate_links( array(
					'prev_text' => '← ' . esc_html__( 'Sebelumnya', 'alkautsar' ),
					'next_text' => esc_html__( 'Berikutnya', 'alkautsar' ) . ' →',
				) );
				?>
			</div>
